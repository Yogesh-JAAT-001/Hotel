...
try {
    $stmt = $pdo->prepare("
        SELECT
            ...
            SUM(CASE WHEN status IN ('Pending', 'Confirmed', 'Checked-in') THEN 1 ELSE 0 END) AS active_bookings,
    ...
    $stmt = $pdo->prepare("
        SELECT
            COUNT(rf.id) AS dining_orders,
            COALESCE(SUM(rf.qty * rf.price), 0) AS dining_spend
        FROM RESERVATION_FOOD rf
        JOIN RESERVATION res ON rf.res_id = res.res_id
        WHERE res.guest_id = ?
    ");
    $stmt->execute([$_SESSION['user_id']]);
    $diningSummary = $stmt->fetch();
    if ($diningSummary) {
        $summary['dining_orders'] = (int)($diningSummary['dining_orders'] ?? 0);
        $summary['dining_spend'] = (float)($diningSummary['dining_spend'] ?? 0);
    }
    ...
    $stmt = $pdo->prepare("
        SELECT
            ...
            rf.price,
            rf.created_at,
            (rf.qty * rf.price) AS line_total,
            fd.title AS item_name,
            res.res_id,
...
$avgBookingValue = ((int)$summary['confirmed_bookings'] > 0)
    ? ((float)$summary['total_spent'] / (int)$summary['confirmed_bookings'])
    : 0.0;
$confirmationRate = ((int)$summary['total_bookings'] > 0)
    ? (((int)$summary['confirmed_bookings'] / (int)$summary['total_bookings']) * 100.0)
    : 0.0;
$diningShare = ((float)$summary['total_spent'] > 0)
    ? (((float)$summary['dining_spend'] / (float)$summary['total_spent']) * 100.0)
    : 0.0;
?>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                ...
                <li class="nav-item"><a class="nav-link" href="reservations.php">Bookings</a></li>
...
<div class="container dashboard-shell">
    <section class="dashboard-banner mb-4" data-reveal>
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
            <div>
                <h2 class="mb-2">Welcome, <?php echo htmlspecialchars($_SESSION['user_name'] ?: 'Yogesh Kumar'); ?></h2>
                <p class="mb-0">Track your stay analytics, dining activity, and premium booking journey in one place.</p>
            </div>
    ...
    <section class="dashboard-kpi-grid mb-4">
        <article class="kpi-card" data-reveal>
            ...
            <div class="kpi-value"><?php echo (int)$summary['active_bookings']; ?></div>
            <div class="kpi-meta">Pending, confirmed, and checked-in stays</div>
        </article>

        <article class="kpi-card" data-reveal>
            <span class="kpi-icon"><i class="fas fa-list-check"></i></span>
            <div class="kpi-label">Total Bookings</div>
            <div class="kpi-value"><?php echo (int)$summary['total_bookings']; ?></div>
            <div class="kpi-meta"><?php echo (int)$summary['confirmed_bookings']; ?> confirmed reservations</div>
        </article>

        <article class="kpi-card" data-reveal>
            <span class="kpi-icon"><i class="fas fa-utensils"></i></span>
            <div class="kpi-label">Dining Orders</div>
            <div class="kpi-value"><?php echo (int)$summary['dining_orders']; ?></div>
            <div class="kpi-meta">₹<?php echo number_format((float)$summary['dining_spend'], 0); ?> dining spend</div>
        </article>

        <article class="kpi-card" data-reveal>
            <span class="kpi-icon"><i class="fas fa-rupee-sign"></i></span>
            <div class="kpi-label">Total Spend</div>
            <div class="kpi-value">₹<?php echo number_format((float)$summary['total_spent'], 0); ?></div>
            <div class="kpi-meta">Reservation + dining spending footprint</div>
        </article>
    </section>

    <section class="analytics-mini-grid mb-4">
        <article class="mini-analytics-card" data-reveal>
            <div class="mini-label">Average Booking Value</div>
            <div class="mini-value">₹<?php echo number_format($avgBookingValue, 0); ?></div>
        </article>
        <article class="mini-analytics-card" data-reveal>
            <div class="mini-label">Confirmation Rate</div>
            <div class="mini-value"><?php echo number_format($confirmationRate, 1); ?>%</div>
        </article>
        <article class="mini-analytics-card" data-reveal>
            <div class="mini-label">Dining Share of Spend</div>
            <div class="mini-value"><?php echo number_format($diningShare, 1); ?>%</div>
        </article>
    </section>
    ...
    <section class="dashboard-panel mb-5" data-reveal>
        <div class="panel-header">
            <h5 class="luxury-header mb-0">Dining Order History</h5>
        </div>
        <div class="panel-body">
            <?php if (empty($dining_orders)): ?>
                <div class="text-center py-4">
                    <i class="fas fa-utensils fa-2x text-muted mb-3"></i>
                    <p class="text-muted mb-0">No dining orders linked to your reservations yet.</p>
                </div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table dashboard-table align-middle mb-0">
                        ...
                        <tbody>
                            <?php foreach ($dining_orders as $order): ?>
                                <tr>
                                    <td>#<?php echo (int)$order['id']; ?></td>
                                    <td>
                                        #<?php echo (int)$order['res_id']; ?><br>
                                        <small class="text-muted"><?php echo htmlspecialchars((string)$order['reservation_status']); ?></small>
                                    </td>
                                    ...
                                    <td>₹<?php echo number_format((float)$order['line_total'], 2); ?></td>
                                    <td><?php echo date('M d, Y', strtotime((string)$order['created_at'])); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
