...
function ensureDiningImagesFolder($projectRoot) {
    $targetDir = $projectRoot . '/IMAGES/live/food';
    if (!is_dir($targetDir)) {
...
try {
    ...
    $defaultPath = 'IMAGES/live/food/default_food.jpg';
    if (!mediaPublicAssetExists('/' . ltrim($defaultPath, '/'))) {
        $defaultPath = 'IMAGES/live/no-food.png';
    }
    ...
    foreach ($rows as $row) {
        ...
        if ($foodName !== '') {
            ...
            $expectedPublic = '/IMAGES/live/food/' . $foodName . '.jpg';
